...
function fetchData() {
    fetch(apiUrl)
        ...
        .catch(error => {
            ...
            document.getElementById('data-section').innerHTML = `<p>Error al cargar los datos: ${error.message}</p>`;
        });
}
    ...
    if (container) {
        if (data.length > 0) {
            container.innerHTML = data.map(item => `
                <div class="card" onclick="showCard('${item.nombre}', ${item.Edad}, '${item["Tipo de Afiliación"]}', '${item["Grupo Familiar"]}', '${item.Celular}', '${item.Correo}', '${item.Partido}', '${new Date(item.Fecha).toLocaleDateString()}', '${item.Hora}', '${item.vendedor}', '${item["Estado del correo Asignación"]}', '${item["Fecha de Envío"]}', '${item["Estado Prospecto"]}', '${item["Fecha de asignación del estado del prospecto"]}', '${item["Envío Prospecto con Estado Aporte por Aporte"]}', '${item["Fecha Envío Prospecto con Estado Aporte por Aporte"]}', '${item["Envío WA"]}', '${item["Fecha envío WA"]}', '${item["ingreso lead"]}')">
...
function showCard(nombre, edad, tipoAfiliacion, grupoFamiliar, celular, correo, partido, fecha, hora, vendedor, estadoCorreo, fechaEnvio, estadoProspecto, fechaAsignacion, envioEstadoAporte, fechaEnvioAporte, envioWA, fechaEnvioWA, ingresoLead) {
    ...
    fetch('card.html')
        ...
        .then(html => {
            ...
            document.getElementById('card-prospect-status').innerText = estadoProspecto;
            document.getElementById('card-prospect-assignment-date').innerText = fechaAsignacion;
            document.getElementById('card-contribution-status').innerText = envioEstadoAporte;
            document.getElementById('card-contribution-send-date').innerText = fechaEnvioAporte;
...
function distributeData(data) {
    ...
    sections.forEach(section => {
        ...
        const filteredData = data.filter(item => {
            return (filter === 'filter2' && item.Edad >= 30) || (filter === 'filter1' && item.Edad < 30);
        });
