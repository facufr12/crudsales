...
document.getElementById("estado_civil").addEventListener("change", function () {
  ...
  const tipoContratacionEsposa = document.getElementById("tipo_contratacion_esposa");
  const categoriaMonotributoEsposaSection = document.getElementById("categoria_monotributo_esposa_section");
  const categoriaMonotributoEsposa = document.getElementById("categoria_monotributo_esposa");
...
function updateContratacion() {
  ...
  document.getElementById("categoria_monotributo_section").style.display = tipoContratacion === "Monotributo" ? "block" : "none";
  document.getElementById("sueldo").style.display = tipoContratacion === "Recibo de Sueldo" ? "block" : "none";
  if (tipoContratacion !== "Recibo de Sueldo") document.getElementById("sueldo_titular").value = 0;
}

document.getElementById("tipo_contratacion").addEventListener("change", updateContratacion);

// Actualizar la sección de contratación de la esposa
function updateContratacionEsposa() {
  const tipoContratacionEsposa = document.getElementById("tipo_contratacion_esposa").value;
  document.getElementById("categoria_monotributo_esposa_section").style.display = tipoContratacionEsposa === "Monotributo" ? "block" : "none";
  document.getElementById("sueldo_e").style.display = tipoContratacionEsposa === "Recibo de Sueldo" ? "block" : "none";
  if (tipoContratacionEsposa !== "Recibo de Sueldo") document.getElementById("sueldo_esposa").value = 0;
}

document.getElementById("tipo_contratacion_esposa").addEventListener("change", updateContratacionEsposa);
...
function updateHijosEdades() {
  document.getElementById("cantidad_hijos").addEventListener("input", function () {
    ...
    for (let i = 0; i < cantidad; i++) {
      edadesHtml += `<input class="form-control" type="number" id="edad_hijo_${i + 1}" name="edad_hijo_${i + 1}" placeholder="Edad Hijo ${i + 1}" required>`;
    }
...
document.getElementById("cotizar").addEventListener("click", function (event) {
  ...
  const jsonData = {
    ...
    membershipWife: document.getElementById("tipo_contratacion_esposa").value || null,
    salaryWife: parseInt(document.getElementById("sueldo_esposa").value, 10) || 0,
    categoryMonotributeWife: document.getElementById("categoria_monotributo_esposa").value || null,
    numberSons: parseInt(document.getElementById("cantidad_hijos").value, 10) || null,
    ...
    categoryMonotribute: document.getElementById("categoria_monotributo").value || null,
  };

  if (document.getElementById("hijos_si").checked) {
    jsonData.sons = true;
    jsonData.ageSons = [];
    for (let i = 1; i <= parseInt(document.getElementById("cantidad_hijos").value, 10); i++) {
      jsonData.ageSons.push(parseInt(document.getElementById(`edad_hijo_${i}`).value, 10));
    }
  } else {
    jsonData.sons = false;
  }
...
function createCardQuote(quotes) {
  ...
  const data = {
    ...
    hijos: document.getElementById("hijos_si").checked ? "Con Hijos" : "Sin Hijos",
  ...
  Object.keys(quotes.WithDs).forEach(key => {
    ...
    const descuentoTitular = data.contratacion === "Recibo de Sueldo" ?
      `<span>Aportes Titular RelDep</span><span class="text-success fw-semibold"> -$${parseFloat(descuentos.Owner).toFixed(0)}</span>` :
      `<span>Aporte/Descuento Monotributo</span><span class="text-success fw-semibold"> -$${parseFloat(descuentos.Owner).toFixed(0)}</span>`;

    const descuentoEsposa = data.contratacion === "Recibo de Sueldo" ?
      `<span>Aportes Esposa/Conyuge RelDep</span><span class="text-success fw-semibold"> -$${parseFloat(descuentos.Wife).toFixed(0)}</span>` :
      `<span>Aporte Monotributo Esposa/Conyuge</span><span class="text-success fw-semibold"> -$${parseFloat(descuentos.Wife).toFixed(0)}</span>`;
        ...
        <div class="card-body border-top pt-2">
          <ul class="list-group list-group-flush mb-0">
            ...
            <li class="d-flex justify-content-between list-group-item px-0">
              ...
              <span class="text-dark fw-semibold">$${quoteWithDs}</span>
            </li>
          </ul>
        </div>
      </div>`;
